...
function row(label, value) {
  return `<tr>
    <td style="padding:8px 14px;border-bottom:1px solid #eee;color:#FFFFFF;font:600 13px/1.5 Arial,sans-serif;white-space:nowrap">${esc(label)}</td>
    <td style="padding:8px 14px;border-bottom:1px solid #eee;color:#FFFFFF;font:400 14px/1.5 Arial,sans-serif">${esc(value) || '-'}</td>
  </tr>`;
}
...
function internalHtml(lead, kind) {
  ...
  return `<div style="background:#FFFFFF;background-color:#FFFFFF;padding:24px">
    <div style="max-width:560px;margin:0 auto;background:#FFFFFF;background-color:#FFFFFF;color:#241A33;border-radius:16px;overflow:hidden;border:1px solid #E7E2F0">
      ...
      <div style="font:700 12px/1.4 Arial,sans-serif;letter-spacing:.14em;text-transform:uppercase;opacity:.85;color:#FFFFFF">ProHealth Programs</div>
      <div style="font:700 20px/1.3 Arial,sans-serif;margin-top:6px;color:#FFFFFF">${heading}</div>
      </div>
